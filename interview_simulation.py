...
CHANNELS            = 1       # Mono
PREFERRED_MIC_INDEX = None    # None = system default; set int to pin a device
...
def record_answer(
    ...
    """
    Record mic audio until silence or max duration.
    ...
    # Pre-check: can we open the mic?
    try:
        test = sd.rec(int(sample_rate * 0.1), samplerate=sample_rate,
                      channels=CHANNELS, dtype="float32", **device_kwargs)
        sd.wait()
    except sd.PortAudioError as exc:
        raise RuntimeError(
            f"Cannot open microphone: {exc}\n"
            "Run list_input_devices() and set PREFERRED_MIC_INDEX."
        ) from exc

    dev_idx  = PREFERRED_MIC_INDEX if PREFERRED_MIC_INDEX is not None else sd.default.device[0]
    ...
    print(f"[REC] Saved {duration:.1f}s -> {Path(tmp.name).name}")
    return tmp.name


def run_mic_test(test_secs: int = 3) -> None:
    """Record a short clip and print RMS level to confirm mic works."""
    print(f"\n[MIC TEST] Recording {test_secs}s — say something now...")
    device_kwargs = {"device": PREFERRED_MIC_INDEX} if PREFERRED_MIC_INDEX is not None else {}
    try:
        rec = sd.rec(int(SAMPLE_RATE * test_secs), samplerate=SAMPLE_RATE,
                     channels=CHANNELS, dtype="float32", **device_kwargs)
        sd.wait()
        rms = float(np.sqrt(np.mean(rec ** 2)))
        status = "OK — audio detected" if rms > 0.002 else "WARNING: very quiet, check mic"
        print(f"[MIC TEST] RMS level : {rms:.4f}  ({status})")
        tmp = tempfile.NamedTemporaryFile(suffix=".wav", delete=False, prefix="mic_test_")
        wavfile.write(tmp.name, SAMPLE_RATE, (rec * 32767).astype(np.int16))
        tmp.close()
        print(f"[MIC TEST] Clip saved : {tmp.name}")
    except sd.PortAudioError as exc:
...
# ─────────────────────────────────────────────────────────────────────────────
STT_CONTEXT_HINTS: list[str] = (
    resume.get("sections", {}).get("skills", []) +
    [p.get("title", "") for p in resume.get("sections", {}).get("projects", [])]
)
print(f"\n[STT] Context hints ({len(STT_CONTEXT_HINTS)} terms): {STT_CONTEXT_HINTS[:10]}")
...
def transcribe_audio(wav_path: str, context_hints: list[str] | None = None) -> str:
    ...
    if context_hints:
        kwargs["context_bias"] = context_hints[:100]   # must be list, not string
